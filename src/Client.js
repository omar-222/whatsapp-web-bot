...
class Client extends EventEmitter {
    ...
    async sendMessage(chatId, content, options = {}) {
        ...
        let internalOptions = {
            ...
            sendMediaAsDocument: options.sendMediaAsDocument,
            caption: options.caption,
            quotedMessageId: options.quotedMessageId,  // Will validate this before sending
            parseVCards: options.parseVCards !== false,
            mentionedJidList: options.mentions || [],
            groupMentions: options.groupMentions,
            invokedBotWid: options.invokedBotWid,
        ...
        // ✅ Validate quoted message before sending
        if (internalOptions.quotedMessageId) {
            const quotedMsg = await this.getMessageById(internalOptions.quotedMessageId);
            if (!quotedMsg) {
                console.warn("Quoted message not found. Sending without a quote.");
                delete internalOptions.quotedMessageId;  // Prevent error
            }
        }
    
        // ✅ Improved Puppeteer Execution
        const newMessage = await this.pupPage.evaluate(async (chatId, message, options, sendSeen) => {
            try {
                const chatWid = window.Store.WidFactory.createWid(chatId);
                const chat = await window.Store.Chat.find(chatWid);
                ...
                // ✅ Validate quoted message inside Puppeteer
                ...
                if (!msg) throw new Error("Message sending failed.");
        ...
        if (newMessage && newMessage.error) {
            console.error("Error sending message:", newMessage.error);
        }
